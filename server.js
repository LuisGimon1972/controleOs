...
db.serialize(() => {
  ...
  `);
/*db.run(`drop table itensOrdemServico`);
db.run(`drop table ordemServico`);
db.run(`drop table clientes`);
//db.run(`drop table receber`);
db.run(`drop table objetosVeiculos`);*/
...
app.get('/api/ordens-servico', (req, res) => {
  const sql = `
    SELECT 
      ...
      IFNULL(os.valorTotalItem, 0) AS valorProdutos,
      IFNULL(os.valorTotalServ, 0) AS valorServicos,
      IFNULL(os.desconto, 0) AS desconto,
      IFNULL(os.acrescimo, 0) AS acrescimo,
      IFNULL(os.adiantamento, 0) AS adiantamento,
      IFNULL(os.valorTotal, 0) AS totalOS,

      COALESCE(
        TRIM(
          ov.tipo ||
          CASE 
            WHEN ov.modelo IS NOT NULL AND ov.modelo <> '' 
            THEN ' - ' || ov.modelo 
            ELSE '' 
          END
        ),
    ...
    LEFT JOIN clientes c ON c.controle = os.clienteId
    LEFT JOIN funcionarios f ON f.controle = os.funcionarioId
...
app.post("/ordemServico", (req, res) => {
  const {
    ...
    formaPagamento, desconto, acrescimo, valorTotalItem, valorTotalServ, valorTotal, adiantamento
  } = req.body;
...
app.get('/api/ordens-servico', (req, res) => {
  const sql = `
    SELECT 
      ...
      os.adiantamento AS Adiantamento,      
...
app.put("/ordemServico/:id", (req, res) => {
  ...
  db.run(sql, [
    numeroOS, clienteId, objetoVeiculoId, funcionarioId, laudo,
    dataAbertura, status, descricao, observacoes,
    formaPagamento, desconto, acrescimo, valorTotalItem, valorTotalServ, valorTotal, adiantamento, id
  ], function (err) {
    if (err) {
...
app.get('/clientes/:id/objetos-veiculos', (req, res) => {
  const clienteId = req.params.id;
    ...
    SELECT 
      ...
      CASE 
        WHEN EXISTS (
          SELECT 1
          FROM ordemServico os
          WHERE os.objetoVeiculoId = ov.id
            AND os.status NOT IN ('FATURADA', 'CANCELADA')
        )
        THEN 1 ELSE 0
      END AS vinculado

    FROM objetosVeiculos ov
    WHERE ov.clienteId = ?
  `;
  ...
  db.all(sql, [clienteId], (err, rows) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ erro: 'Erro ao buscar objetos' });
    }
    res.json(rows);
  });
});
...
app.delete('/clientes/:clienteId/objetos-veiculos', (req, res) => {
  ...
  const sql = `
    DELETE FROM objetosVeiculos
    WHERE clienteId = ?
      AND id IN (${placeholders})
      AND id NOT IN (
        SELECT os.objetoVeiculoId
        FROM ordemServico os
        WHERE os.status NOT IN ('FATURADA', 'CANCELADA')
      )
  `;

  db.run(sql, [clienteId, ...ids], function (err) {
    if (err) {
      console.error(err);
      return res.status(500).json({ erro: 'Erro ao remover objetos' });
    }

    if (this.changes === 0) {
      return res.status(409).json({
        erro: 'Existem objetos vinculados a OS ativas'
      });
    }
...
app.delete('/objetos-veiculos/:id', (req, res) => {
  const { id } = req.params;
  const sql = `
    DELETE FROM objetosVeiculos
    WHERE id = ?
  ...
  db.run(sql, [id, id], function (err) {
    if (err) {
      console.error(err);
      return res.status(500).json({ erro: 'Erro ao remover objeto' });
    }

    if (this.changes === 0) {
      return res.status(409).json({
        erro: 'Objeto vinculado a OS ativa'
      });
    }
