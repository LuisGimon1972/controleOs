...
function renderizarTabelaComPaginacao() {
  ...
  parcelasPagina.forEach(parcela => {
    ...
    linha.innerHTML = `
      ...
      <td style="text-align: center;">${parcela.descricao}</td>
...
const camposPagamento8 = ['dinheiro', 'cartaoDebito', 'cartaoCredito'];
...
camposPagamento8.forEach((id, index) => {
  ...
  campo.addEventListener('keydown', (event) => {
    if (event.key === 'Enter') {
      ...
      processarCampo(id);
      const proximoCampo = document.getElementById(camposPagamento8[index + 1]);
      if (proximoCampo) {
        proximoCampo.focus();
      }
    }
...
document.addEventListener('DOMContentLoaded', () => {
  const btnFinalizafc = document.getElementById('btnFinalizarFc');
  if (btnFinalizafc) {
    btnFinalizafc.addEventListener('click', () => {
  ...
  async function FinalOperacaofc() {
  const dinheiro = arredondar(+document.getElementById('dinheiro').value || 0);
  const debito   = arredondar(+document.getElementById('cartaoDebito').value || 0);
  const credito  = arredondar(+document.getElementById('cartaoCredito').value || 0);
  ...
  if (totalPago < total) {
    faltaEl.value = (total - totalPago).toFixed(2);
    trocoEl.value = '0.00';
    showToast(`Pagamento insuficiente. Total: R$ ${total.toFixed(2)} | Pago: R$ ${totalPago.toFixed(2)}`, 2000);
    return;
  }

  // 🔴 Cartões não geram troco
  if (totalPago > total && dinheiro === 0) {
    faltaEl.value = '0.00';
    trocoEl.value = '0.00';
    showToast('O valor total dos cartões excede o valor da venda.', 2500);
    return;
  }
  ...
  showToast('Parcelas quitadas com sucesso!', 2500);

  await salvarLancamentosCaixaReceber();
  await pagarParcelasSelecionadas();

  
  swti = 9;
  moduloAtivo = 'finalizado';
