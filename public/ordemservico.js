...
function cadordemos() {  
  ...
  function adicionarItemVendaOs(produto, quantidade) {
    ...
    selecclieno = document.getElementById('selectClienteos').value
...
function recalcularTotalOs() {
  ...
  const totalFinal = totalBase - desconto - adiantamentos + acrescimo;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const adiantamentoEL = document.getElementById('adiantamento');
  ...
  if (adiantamentoEL) adiantamentoEL.addEventListener('input', recalcularTotalOs);
});
...
async function salvarOrdemServico() {  
  try {    
    ...
    const adiantamento = adiantamentoos
    const ordemServico = {
      numeroOS,
      clienteId,
      objetoVeiculoId,
...
function calcularOperacaoOs() {  
  ...
  if(adiantamentoos>0 && dinheiro<adiantamentoos){
    showToast(`O monto do dinhiero não pode ser menor que o adiantamento. Adiantamento: R$ ${adiantamentoos.toFixed(2)}`,
      2000
    );
    document.getElementById('dinheiro').value = adiantamentoos.toFixed(2); 
    document.getElementById('dinheiro').focus()
    return;
  }
  
  if (adiantamentoos > 0 && dinheiro >= adiantamentoos){
    dinheiro = dinheiro - adiantamentoos   
  }
...
function imprimirVendaOSCad(venda = {}) {
  ...
  const adianta = adiantamentoos || 0;  
  ...
  const gerarEspeciesHTML = () => `
    <div style="margin-top: 6px; font-size: 12px;">
      ...
      `).join('')}
      <div style="display: flex; justify-content: space-between;">
        <span>Troco:</span> <span>${formatarMoeda(trocoos)}</span>
      </div>
    </div>
  `;
...
function habilitarcontrolesOs()
  ...
  document.getElementById('ladiantamento').style.display = 'inline-block';
  document.getElementById('adiantamento').style.display = 'inline-block';
  ...
  document.getElementById('ladiantamentox').style.display = 'none';
  document.getElementById('adiantamentox').style.display = 'none';
  }

const checkboxParceladoso = document.getElementById('parceladoCredito');
const inputCartaoCreditoso = document.getElementById('cartaoCredito');
const detalhesParcelamentoso = document.getElementById('detalhesParcelamento');
const inputQtdParcelasso = document.getElementById('qtdParcelas');
const inputVencimentoso = document.getElementById('vencimentoDia');

async function atualizarVisibilidadeParcelamentoOso() {
  const parceladoso = checkboxParceladoso.checked;
  const valorCartaoo = parseFloat(inputCartaoCreditoso.value) || 0;
  const mostrar = parceladoso && valorCartaoo > 0;
  detalhesParcelamentoso.style.display = mostrar ? 'block' : 'none';

  if (!mostrar) {
    inputQtdParcelasso.value = '1';
    inputVencimentoso.value = '';
    return;
  }
  
  if (controleSelecionadoos === 1) return;

  const disponibilidade = await calcularDisponibilidadeOso(controleSelecionadoos);
  
  if (disponibilidade === null) return;

  if (valorCartaoo > disponibilidade) {
    showToast(
      `Crédito insuficiente! Disponível: R$ ${disponibilidade.toFixed(2)}`,
      3500
    );
    checkboxParceladoso.checked = false;
    detalhesParcelamentoso.style.display = 'none';
  }
}

checkboxParceladoso.addEventListener('change', atualizarVisibilidadeParcelamentoOso);
inputCartaoCreditoso.addEventListener('input', atualizarVisibilidadeParcelamentoOso);

async function mostrarParcelasAbertasOso(clienteId) {
  try {
...
async function buscarLimiteClienteOso(clienteId) {
  try {
...
async function calcularDisponibilidadeOso(clienteId) {
  const deuda = await mostrarParcelasAbertasOso(clienteId);
  const limitec = await buscarLimiteClienteOso(clienteId);
  if (deuda === null || limitec === null) return null;
  return limitec - deuda;
}

function gerarParcelasNaTelaOso(parcelas, diaVencimento, valorCredito) {
...
const qtdInputso = document.getElementById('qtdParcelas');
const vencInputso = document.getElementById('vencimentoDia');

function tentarGerarParcelasOso() {
  const ehParcelado = document.getElementById('parceladoCredito').checked;
  if (!ehParcelado) {
  ...
  if (parcelas >= 1 && vencimento >= 1 && vencimento <= 31 && valorCredito > 0) {
    gerarParcelasNaTelaOso(parcelas, vencimento, valorCredito);
  } else {
    document.getElementById('parcelasGeradas').innerHTML = '';
  }
}

qtdInputso.addEventListener('input', tentarGerarParcelasOso);
vencInputso.addEventListener('input', tentarGerarParcelasOso); 

async function salvarParcelasReceberOso() {
