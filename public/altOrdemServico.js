...
function preencherCamposOs(os) {
  ...
  document.getElementById('descoso').value = isNaN(desconto) ? '0.00' : desconto.toFixed(2);
  document.getElementById('ascoso').value =  isNaN(acrescimo) ? '0.00' : acrescimo.toFixed(2);
  document.getElementById('adiantamentox').value =  isNaN(adiantamento) ? '0.00' : adiantamento.toFixed(2);
  document.getElementById('totalgeralos').value = (os.valorTotal ?? 0).toFixed(2);    
  descontosos = os.desconto
  acrescimosos = os.acrescimo  
  adiantamentooso = os.adiantamento
  const selecth = document.getElementById('selectClienteos');
  controleSelecionadoos = document.getElementById('selectClienteos').value;  
...
function recalcularTotalOsOs() {
  ...
  if (adiantamentos >= totalGeralOsx) {    
    showToastl("⚠️ O adiantamento não pode ser maior que o total da Ordem de Serviço!", 2500);    
    totalGeralInput.value = totalBase.toFixed(2);    
    adiantamentoInput.style.border = "2px solid red";    
    setTimeout(() => {
      adiantamentoInput.value = '';
      adiantamentoInput.style.border = '';
      const totalFinal = totalGeralOsx + acrescimo;
  ...
  totalGeralInput.value = (totalGeralOsx - desconto - adiantamentos + acrescimo).toFixed(2);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const adiantamentoEL = document.getElementById('adiantamentox');
  ...
  if (adiantamentoEL) adiantamentoEL.addEventListener('input', recalcularTotalOsOs);
});
...
function FinalizarOperacaoOsOs() {  
  ...
  if (adiantamentooso > 0) {
    const campo = document.getElementById('dinheiro');
    campo.value = adiantamentooso.toFixed(2);    
...
`;  
  ...
  os_totalGeral = os_totalGeral + acrescimosos - adiantamentooso - descontosos
...
function calcularOperacaoOsOs() {
  ...
  let dinheiro = arredondar(dinheiroInput);
  ...
  if(adiantamentooso>0 && dinheiro<adiantamentooso){
    ...
    document.getElementById('dinheiro').value = adiantamentooso.toFixed(2); 
    document.getElementById('dinheiro').focus()
    return;
  }
  
  if (adiantamentooso > 0 && dinheiro >= adiantamentooso){
    dinheiro = dinheiro - adiantamentooso   
  }
...
function FinalOperacaoOsOs() {    
  let dinheiro = arredondar(parseFloat(document.getElementById('dinheiro').value) || 0);
  ...
  if (totalPago < total) {
    ...
    faltaEl.value = falta.toFixed(2);
    trocoEl.value = "0.00";
    showToast(`Pagamento insuficiente. Total: R$ ${total} | Pago: R$ ${totalPago.toFixed(2)}`, 2000);
    return;
  }
  ...
  showToast("Ordem de Serviço finalizada com sucesso!", 3500);  
  salvarLancamentosCaixaOsOs();     
  const ehParcelado = document.getElementById('parceladoCredito').checked;
  if (ehParcelado && credito > 0) {
    salvarParcelasReceberOs()
  }    
...
function limparCamposOsOs() {
  ...
  if (titulo) titulo.textContent = 'Ordem de Serviço';
...
function imprimirVendaOS(venda = {}) {
  ...
  const gerarCorpoCupom = () => `
    ${gerarCabecalho()}
    <div style="border-top: 1px dashed #000; margin: 4px 0;"></div>
    <div style="font-size: 12px; line-height: 1.4; font-family: monospace;">
...
const checkboxParcelados = document.getElementById('parceladoCredito');
const inputCartaoCreditos = document.getElementById('cartaoCredito');
const detalhesParcelamentos = document.getElementById('detalhesParcelamento');
const inputQtdParcelass = document.getElementById('qtdParcelas');
const inputVencimentos = document.getElementById('vencimentoDia');

async function atualizarVisibilidadeParcelamentoOs() {
  const parcelados = checkboxParcelados.checked;
  const valorCartao = parseFloat(inputCartaoCreditos.value) || 0;
  const mostrar = parcelados && valorCartao > 0;
  detalhesParcelamentos.style.display = mostrar ? 'block' : 'none';
  
  if (!mostrar) {
    inputQtdParcelass.value = '1';
    inputVencimentos.value = '';
    return;
  }  
  if (controleSelecionadoos === 1) return;

  const disponibilidade = await calcularDisponibilidadeOs(controleSelecionadoos);
  
  if (disponibilidade === null) return;

  if (valorCartao > disponibilidade) {
    showToast(
      `Crédito insuficiente! Disponível: R$ ${disponibilidade.toFixed(2)}`,
      3500
    );
    checkboxParcelados.checked = false;
    detalhesParcelamentos.style.display = 'none';
  }
}

checkboxParcelados.addEventListener('change', atualizarVisibilidadeParcelamentoOs);
inputCartaoCreditos.addEventListener('input', atualizarVisibilidadeParcelamentoOs);

async function mostrarParcelasAbertasOs(clienteId) {
  try {
    const res = await fetch(`/clientes/${clienteId}/total-aberto`);
    const data = await res.json();
    return parseFloat(data.total_aberto) || 0;
  } catch (err) {
    showToast('Erro ao buscar parcelas em aberto', 3000);
    return null;
  }
}

async function buscarLimiteClienteOs(clienteId) {
  try {
    const res = await fetch(`/clientes/${clienteId}/limite`);
    if (!res.ok) throw new Error();
    const { limite } = await res.json();
    return parseFloat(limite) || 0;
  } catch {
    showToast('Erro ao buscar limite do cliente', 3000);
    return null;
  }
}

async function calcularDisponibilidadeOs(clienteId) {  
  const deuda = await mostrarParcelasAbertasOs(clienteId);
  const limitec = await buscarLimiteClienteOs(clienteId);
  if (deuda === null || limitec === null) return null;
  return limitec - deuda;
}

function gerarParcelasNaTelaOs(parcelas, diaVencimento, valorCredito) {
...
const qtdInputs = document.getElementById('qtdParcelas');
const vencInputs = document.getElementById('vencimentoDia');

function tentarGerarParcelasOs() {
  const ehParcelado = document.getElementById('parceladoCredito').checked;
  if (!ehParcelado) {
  ...
  if (parcelas >= 1 && vencimento >= 1 && vencimento <= 31 && valorCredito > 0) {
    gerarParcelasNaTelaOs(parcelas, vencimento, valorCredito);
  } else {
    document.getElementById('parcelasGeradas').innerHTML = '';
  }
}

qtdInputs.addEventListener('input', tentarGerarParcelasOs);
vencInputs.addEventListener('input', tentarGerarParcelasOs); 

async function salvarParcelasReceberOs() {
  const funcionario = funcionarioSelecionado;
  const cliente_id = controleSelecionadoos || 1;
  const descricao = nosx;
